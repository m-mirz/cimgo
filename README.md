...
a Go `Check<...>` function under `shaclgen/`. The 205 SPARQL constraints (195 
...
that both tools' importers now agree on how many distinct constraints the schema actually
...
9815, cimoxide generates 4141), not a counting discrepancy.
...
|-----------------|----------------|
| `C:301:EQ:LoadResponseCharacteristic.exponentModel:coefficient`, `C:301:EQ:LoadResponseCharacteristic.exponentModel:coefficientSum` | Equipment (EQ) — `CheckLoadResponseCharacteristicExponentModel` (`validation/sparql_equipment.go`) tags both branches with the `:exponent` name; the coefficient-model branch should use these instead |
| `C:456:SSH:TapChanger.step:value` | Steady State Hypothesis (SSH) — `61970-456_SteadyStateHypothesis-AP-Con-Complex-NotSolvedMAS-SHACL.ttl` |
| `C:302:DY:GovHydro4.bgv0:valueRange` … `C:302:DY:GovHydro4.bgv5:valueRange` | Dynamics (DY) — `61970-302_Dynamics-AP-Con-Complex-SHACL.ttl` |
| `C:456:SV:SvVoltage.v:limits` | State Variables (SV) — `61970-456_StateVariables-AP-Con-Complex-SolvedMAS-SHACL.ttl` |
