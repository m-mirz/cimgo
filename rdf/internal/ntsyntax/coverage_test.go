...
func TestTermURIRef(t *testing.T) {
	u, _ := term.NewURIRef("http://example.org/s")
	s, err := Term(u)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLiteralWithLang(t *testing.T) {
	l := term.NewLiteral("hello", term.WithLang("en"))
	s := Literal(l)
	if s != `"hello"@en` {
		t.Errorf("got %s", s)
	}
...
func TestLiteralWithDirLang(t *testing.T) {
	l := term.NewLiteral("hello", term.WithLang("ar"), term.WithDir("rtl"))
	s := Literal(l)
	if s != `"hello"@ar--rtl` {
		t.Errorf("got %s", s)
	}
...
func TestLiteralWithDatatype(t *testing.T) {
	l := term.NewLiteral("42", term.WithDatatype(term.XSDInteger))
	s := Literal(l)
