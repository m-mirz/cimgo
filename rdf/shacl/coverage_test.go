...
func TestFromRDFLibDirLangString(t *testing.T) {
	rdflibTerm := term.NewLiteral("hello", term.WithLang("ar"), term.WithDir("rtl"))
...
func TestFromRDFLibDirLangStringCov2(t *testing.T) {
	...
	lit := term.NewLiteral("مرحبا", term.WithLang("ar"), term.WithDir("rtl"))
	term := fromRDFLib(lit)
	if term.language == "" {
