...
func TestEvalGraphPattern(t *testing.T) {
	...
	ng := graph.NewGraph()
	ng.Add(s, p, term.NewLiteral("ng-v"))
	...
	q.NamedGraphs = map[string]*graph.Graph{
...
func TestEvalFilterBoolean(t *testing.T) {
	g := graph.NewGraph()
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/v")
	g.Add(s, p, term.NewLiteral(true))
	g.Add(s, p, term.NewLiteral(false))
...
func TestEvalDateFunctions(t *testing.T) {
	g := graph.NewGraph()
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/date")
	g.Add(s, p, term.NewLiteral("2024-03-15T10:30:45Z", term.WithDatatype(term.XSDDateTime)))
...
func TestEBVIntegerZero(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/v")
	g.Add(s, p, term.NewLiteral(int64(0), term.WithDatatype(term.XSDInteger)))
	g.Add(s, p, term.NewLiteral(int64(5), term.WithDatatype(term.XSDInteger)))
	r, _ := Query(g, `PREFIX ex: <http://example.org/> SELECT ?v WHERE { ?s ex:v ?v . FILTER(?v) }`)
	if len(r.Bindings) != 1 {
...
func TestEBVFloat(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/v")
	g.Add(s, p, term.NewLiteral("0.0", term.WithDatatype(term.XSDFloat)))
	g.Add(s, p, term.NewLiteral("3.14", term.WithDatatype(term.XSDFloat)))
	r, _ := Query(g, `PREFIX ex: <http://example.org/> SELECT ?v WHERE { ?s ex:v ?v . FILTER(?v) }`)
	if len(r.Bindings) != 1 {
...
func TestEBVDecimal(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/v")
	g.Add(s, p, term.NewLiteral("0.0", term.WithDatatype(term.XSDDecimal)))
	g.Add(s, p, term.NewLiteral("1.5", term.WithDatatype(term.XSDDecimal)))
	r, _ := Query(g, `PREFIX ex: <http://example.org/> SELECT ?v WHERE { ?s ex:v ?v . FILTER(?v) }`)
	if len(r.Bindings) != 1 {
...
func TestEBVDefaultLiteral(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/v")
	custom := term.NewURIRefUnsafe("http://example.org/mytype")
	g.Add(s, p, term.NewLiteral("somevalue", term.WithDatatype(custom)))
	g.Add(s, p, term.NewLiteral("", term.WithDatatype(custom)))
	r, _ := Query(g, `PREFIX ex: <http://example.org/> SELECT ?v WHERE { ?s ex:v ?v . FILTER(?v) }`)
	if len(r.Bindings) != 1 {
...
func TestSumDecimal(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/v")
	g.Add(s, p, term.NewLiteral("1.5", term.WithDatatype(term.XSDDecimal)))
	g.Add(s, p, term.NewLiteral("2.5", term.WithDatatype(term.XSDDecimal)))
...
func TestAvgDecimal(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/v")
	g.Add(s, p, term.NewLiteral("2.5", term.WithDatatype(term.XSDDecimal)))
	g.Add(s, p, term.NewLiteral("3.5", term.WithDatatype(term.XSDDecimal)))
...
func TestExtractDatePartDateOnly(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/date")
	g.Add(s, p, term.NewLiteral("2024-06-15", term.WithDatatype(term.XSDDate)))
	r, err := Query(g, `
		PREFIX ex: <http://example.org/>
		SELECT ?y ?m ?d WHERE {
			?s ex:date ?dt .
...
func TestExtractDatePartNoTZ(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/date")
	g.Add(s, p, term.NewLiteral("2024-03-15T10:30:45", term.WithDatatype(term.XSDDateTime)))
	r, err := Query(g, `
		PREFIX ex: <http://example.org/>
		SELECT ?h ?min WHERE {
			?s ex:date ?dt .
...
func TestParseLiteralStringLangTag(t *testing.T) {
	g := graph.NewGraph()
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/label")
...
func TestParseLiteralStringTripleQuote(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/label")
	g.Add(s, p, term.NewLiteral("hello world"))
...
func TestClearNamed(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/p")
	g1 := graph.NewGraph()
	g1.Add(s, p, term.NewLiteral("v"))
	ds := &Dataset{
		Default:     graph.NewGraph(),
		NamedGraphs: map[string]*graph.Graph{"http://example.org/g1": g1},
	}
...
func TestDropAll(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/p")
	def := graph.NewGraph()
	def.Add(s, p, term.NewLiteral("default"))
	g1 := graph.NewGraph()
	g1.Add(s, p, term.NewLiteral("named"))
	ds := &Dataset{
		Default:     def,
		NamedGraphs: map[string]*graph.Graph{"http://example.org/g1": g1},
	}
...
func TestLoadSilentNoLoader(t *testing.T) {
	ds := &Dataset{Default: graph.NewGraph(), NamedGraphs: map[string]*graph.Graph{}}
...
func TestAddGraphs(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/p")
	src := graph.NewGraph()
	src.Add(s, p, term.NewLiteral("v"))
	dst := graph.NewGraph()
	ds := &Dataset{
		Default:     graph.NewGraph(),
		NamedGraphs: map[string]*graph.Graph{"http://example.org/src": src, "http://example.org/dst": dst},
	}
...
func TestCopyGraphs(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/p")
	src := graph.NewGraph()
	src.Add(s, p, term.NewLiteral("v1"))
	dst := graph.NewGraph()
	dst.Add(s, p, term.NewLiteral("old"))
	ds := &Dataset{
		Default:     graph.NewGraph(),
		NamedGraphs: map[string]*graph.Graph{"http://example.org/src": src, "http://example.org/dst": dst},
	}
...
func TestMoveSameGraph(t *testing.T) {
	...
	ds := &Dataset{
		Default:     graph.NewGraph(),
		NamedGraphs: map[string]*graph.Graph{"http://example.org/g": g},
	}
...
<sparql xmlns="http://www.w3.org/2005/sparql-results#"
  ...
  <results>
	...
	lit := result.Bindings[0]["v"].(term.Literal)
	if lit.Language() != "ar" {
...
func TestDateComparisonFails(t *testing.T) {
	g := graph.NewGraph()
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/d")
	// Add a date and compare with another date
	g.Add(s, p, term.NewLiteral("2024-01-01", term.WithDatatype(term.XSDDate)))
	g.Add(s, p, term.NewLiteral("2025-01-01", term.WithDatatype(term.XSDDate)))
	r, err := Query(g, `
		PREFIX ex: <http://example.org/>
		SELECT ?d WHERE {
			?s ex:d ?d .
...
func TestParseDateTimeTime(t *testing.T) {
	g := graph.NewGraph()
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/t")
	g.Add(s, p, term.NewLiteral("10:30:45", term.WithDatatype(term.XSDTime)))
	g.Add(s, p, term.NewLiteral("20:00:00", term.WithDatatype(term.XSDTime)))
	r, err := Query(g, `
		PREFIX ex: <http://example.org/>
		SELECT ?t WHERE {
			?s ex:t ?t .
...
func TestParseDateTimeComparison(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/d")
	g.Add(s, p, term.NewLiteral("2024-01-01T10:00:00Z", term.WithDatatype(term.XSDDateTime)))
	g.Add(s, p, term.NewLiteral("2025-01-01T10:00:00Z", term.WithDatatype(term.XSDDateTime)))
	r, err := Query(g, `
		PREFIX ex: <http://example.org/>
		PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
		SELECT ?d WHERE {
