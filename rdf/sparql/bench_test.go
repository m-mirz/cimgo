...
func setupBenchGraph(n int) *graph.Graph {
	g := graph.NewGraph()
	ns := "http://example.org/"
	for i := range n {
		s := term.NewURIRefUnsafe(ns + "s" + string(rune('0'+i%10)))
		p := term.NewURIRefUnsafe(ns + "p")
		o := term.NewLiteral(i, term.WithDatatype(term.XSDInteger))
		g.Add(s, p, o)
	}
	return g
}
...
func BenchmarkSelectWithRegex(b *testing.B) {
	g := graph.NewGraph()
	ns := "http://example.org/"
	p := term.NewURIRefUnsafe(ns + "name")
	for i := range 500 {
		s := term.NewURIRefUnsafe(ns + "s" + string(rune('A'+i%26)))
		g.Add(s, p, term.NewLiteral("Alice"+string(rune('0'+i%10))))
	}
	q := `PREFIX ex: <http://example.org/>
