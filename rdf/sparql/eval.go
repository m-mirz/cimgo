...
func EvalQuery(g *graph.Graph, q *ParsedQuery, initBindings map[string]term.Term) (*Result, error) {
	...
	var solutions []map[string]term.Term
	...
	case "CONSTRUCT":
		...
		if len(q.OrderBy) > 0 {
			slices.SortFunc(solutions, func(a, b map[string]term.Term) int {
...
func evalSelect(g *graph.Graph, q *ParsedQuery, solutions []map[string]term.Term) (*Result, error) {
	// Aggregation
	...
	if q.Distinct {
		...
		var unique []map[string]term.Term
	...
	if (q.Limit >= 0 || q.Offset > 0) &&
		...
		if tp := extractSimpleBGP(q.Where); tp != nil {
			if qs, ok := g.Store().(store.QueryableStore); ok {
				...
				solutions = nil
				qs.TriplesWithLimit(pat, g.Identifier(), limit, q.Offset)(func(t term.Triple) bool {
					row := make(map[string]term.Term)
...
projectVars:
	...
	// Project
	if vars != nil {
		projected := make([]map[string]term.Term, len(solutions))
		for i, s := range solutions {
			row := make(map[string]term.Term)
			for _, v := range vars {
				if val, ok := s[v]; ok {
...
func evalAggregation(g *graph.Graph, q *ParsedQuery, solutions []map[string]term.Term) []map[string]term.Term {
			...
			if tp := extractSimpleBGP(q.Where); tp != nil {
				if qs, okQ := g.Store().(store.QueryableStore); okQ {
					...
					row := map[string]term.Term{
						pe.Var: term.NewLiteral(cnt, term.WithDatatype(term.XSDInteger)),
					}
					return []map[string]term.Term{row}
				}
			}
		}
	}
	...
	type group struct {
		keyBinds map[string]term.Term
		members  []map[string]term.Term
	}
	...
	for _, s := range solutions {
		var keyParts []string
		keyBinds := make(map[string]term.Term)
	...
	if len(groups) == 0 && hasAggregates(q) && len(q.GroupBy) == 0 {
		groups[""] = &group{keyBinds: map[string]term.Term{}}
		order = append(order, "")
	}

	var result []map[string]term.Term
	for _, k := range order {
		grp := groups[k]
...
func evalAggExpr(expr Expr, group []map[string]term.Term, prefixes map[string]string) term.Term {
	if expr == nil {
		return nil
	}
	switch e := expr.(type) {
	...
	case *LiteralExpr:
		return e.Value
	case *IRIExpr:
		return term.NewURIRefUnsafe(e.Value)
	}
	return nil
}

func evalAggregate(fe *FuncExpr, group []map[string]term.Term, prefixes map[string]string) term.Term {
	var vals []term.Term
	hasError := false
	for _, s := range group {
		if fe.Star {
			vals = append(vals, term.NewLiteral(1))
	...
	if fe.Distinct {
		if fe.Star {
			...
			if fe.Name == "COUNT" {
				return term.NewLiteral(count, term.WithDatatype(term.XSDInteger))
			}
		}
		seen := make(map[string]bool)
		var unique []term.Term
		for _, v := range vals {
			k := v.N3()
			if !seen[k] {
				seen[k] = true
	...
	switch fe.Name {
	case "COUNT":
		return term.NewLiteral(len(vals), term.WithDatatype(term.XSDInteger))
	case "SUM":
		if hasError {
			return nil
		}
		...
		for _, v := range vals {
			...
			if l, ok := v.(term.Literal); ok && l.Datatype() == term.XSDDecimal {
				hasDecimal = true
			}
		}
		if allInt {
			...
			if sum > float64(math.MaxInt64) || sum < float64(math.MinInt64) || math.IsNaN(sum) || math.IsInf(sum, 0) {
				return term.NewLiteral(sum)
			}
			return term.NewLiteral(int64(sum), term.WithDatatype(term.XSDInteger))
		}
		if hasDecimal {
			return term.NewLiteral(formatDecimal(sum), term.WithDatatype(term.XSDDecimal))
		}
		return term.NewLiteral(sum)
	case "AVG":
		if hasError {
		...
		if hasDecimal {
			return term.NewLiteral(formatDecimal(avg), term.WithDatatype(term.XSDDecimal))
		}
		return term.NewLiteral(avg)
	case "MIN":
		if len(vals) == 0 {
			return nil
		}
	...
	case "GROUP_CONCAT":
		...
		return term.NewLiteral(strings.Join(parts, fe.Separator))
	case "SAMPLE":
		if len(vals) > 0 {
			return vals[0]
		}
...
func evalConstruct(g *graph.Graph, q *ParsedQuery, solutions []map[string]term.Term) (*Result, error) {
	...
	for _, sol := range solutions {
		for _, tmpl := range q.Construct {
			...
			subj, okS := s.(term.Subject)
			pred, okP := p.(term.URIRef)
			if !okS || !okP {
				continue
			}
...
func resolveTemplateValue(s string, bindings map[string]term.Term, prefixes map[string]string) term.Term {
...
func resolveTermRef(s string, prefixes map[string]string) term.Term {
	if s == "" {
		return nil
	}
	// Triple term
	...
	if strings.HasPrefix(s, "<") && strings.HasSuffix(s, ">") {
		...
		return term.NewURIRefUnsafe(iri)
	}
	...
	if len(s) > 0 && (s[0] >= '0' && s[0] <= '9' || s[0] == '+' || s[0] == '-') {
		if strings.ContainsAny(s, "eE") {
			return term.NewLiteral(s, term.WithDatatype(term.XSDDouble))
		}
	...
	if idx := strings.Index(s, ":"); idx >= 0 {
		...
		if ns, ok := prefixes[prefix]; ok {
			return term.NewURIRefUnsafe(ns + local)
		}
	}
	return nil
}
...
func evalPattern(g *graph.Graph, pattern Pattern, prefixes map[string]string, namedGraphs map[string]*graph.Graph) []map[string]term.Term {
	if pattern == nil {
	...
	case *BGP:
		return evalBGP(g, p.Triples, map[string]term.Term{}, prefixes)
	...
	case *ValuesPattern:
		...
		for _, row := range p.Values {
			b := make(map[string]term.Term)
			for i, v := range p.Vars {
				if i < len(row) && row[i] != nil {
					val := row[i]
					// Resolve relative IRIs in VALUES against base
					if u, ok := val.(term.URIRef); ok && base != "" && !strings.Contains(u.Value(), ":") {
						val = term.NewURIRefUnsafe(resolveRelativeIRI(base, u.Value()))
					}
					b[v] = val
				}
			}
	...
	case *SubqueryPattern:
		...
		if namedGraphs != nil {
			ng := make(map[string]*graph.Graph, len(namedGraphs))
...
func evalGraphPattern(g *graph.Graph, gp *GraphPattern, prefixes map[string]string, namedGraphs map[string]*graph.Graph) []map[string]term.Term {
	if namedGraphs == nil {
	...
	if isVar {
		...
		varName := graphName[1:]
		var results []map[string]term.Term
		for name, namedG := range namedGraphs {
			graphURI := term.NewURIRefUnsafe(name)
			// Push graph binding into inner pattern evaluation
			initBindings := map[string]term.Term{varName: graphURI}
	...
	graphIRI := ""
	if u, ok := resolved.(term.URIRef); ok {
		graphIRI = u.Value()
	} else {
...
func minusCompatible(a, b map[string]term.Term) bool {
	shared := false
	for k, va := range a {
		if vb, ok := b[k]; ok {
			shared = true
...
func evalPatternWithBindings(g *graph.Graph, pattern Pattern, bindings map[string]term.Term, prefixes map[string]string, namedGraphs map[string]*graph.Graph) []map[string]term.Term {
	switch p := pattern.(type) {
	case *BGP:
		return evalBGP(g, p.Triples, bindings, prefixes)
	default:
		results := evalPattern(g, pattern, prefixes, namedGraphs)
		var compatible []map[string]term.Term
...
func evalBGP(g *graph.Graph, triples []Triple, bindings map[string]term.Term, prefixes map[string]string) []map[string]term.Term {
	if len(triples) == 0 {
		return []map[string]term.Term{bindings}
	}
	...
	g.Triples(subj, pred, obj)(func(t term.Triple) bool {
		...
		if strings.HasPrefix(tp.Subject, "?") {
			...
			if tt, ok := t.Subject.(term.TripleTerm); ok {
...
func evalPathTriple(g *graph.Graph, tp Triple, rest []Triple, bindings map[string]term.Term, prefixes map[string]string) []map[string]term.Term {
	gg := (*graph.Graph)(g)
...
func resolvePatternTerm(s string, bindings map[string]term.Term, prefixes map[string]string) term.Term {
	if strings.HasPrefix(s, "?") {
		v := s[1:]
		if val, ok := bindings[v]; ok {
			return val
...
func resolveTripleTermPattern(s string, bindings map[string]term.Term, prefixes map[string]string) term.Term {
	...
	subj, ok := st.(term.Subject)
	if !ok {
		return nil
	}
	pred, ok := pt.(term.URIRef)
	if !ok {
		return nil
	}
	return term.NewTripleTerm(subj, pred, ot)
}
...
func matchTripleTermPattern(tt term.TripleTerm, pattern string, bindings map[string]term.Term, prefixes map[string]string) map[string]term.Term {
	...
	for _, c := range components {
		if strings.HasPrefix(c.pattern, "?") {
			...
			actualTT, ok := c.actual.(term.TripleTerm)
			if !ok {
				return nil
			}
...
func evalExpr(expr Expr, bindings map[string]term.Term, prefixes map[string]string) term.Term {
	if expr == nil {
		return nil
	}
...
func evalExprWithGraph(expr Expr, bindings map[string]term.Term, prefixes map[string]string, g *graph.Graph, namedGraphs map[string]*graph.Graph) term.Term {
	if expr == nil {
		return nil
	}
	switch e := expr.(type) {
...
func evalBinaryOp(op string, left, right term.Term) term.Term {
	switch op {
	case "=":
		if left == nil || right == nil {
			return term.NewLiteral(false)
		}
		return term.NewLiteral(termValuesEqual(left, right))
	case "!=":
		if left == nil || right == nil {
			return term.NewLiteral(true)
		}
	...
	case "&&":
		return term.NewLiteral(effectiveBooleanValue(left) && effectiveBooleanValue(right))
	case "||":
		return term.NewLiteral(effectiveBooleanValue(left) || effectiveBooleanValue(right))
	case "+", "-", "*", "/":
		if left == nil || right == nil {
		...
		if isIntegral(left) && isIntegral(right) && op != "/" {
			return term.NewLiteral(int64(result))
		}
		if isDecimal(left) || isDecimal(right) {
			return term.NewLiteral(formatDecimal(result), term.WithDatatype(term.XSDDecimal))
		}
		return term.NewLiteral(result)
	}
	return nil
}

func evalUnaryOp(op string, arg term.Term) term.Term {
	switch op {
	case "!":
		if arg == nil {
		...
		if l, ok := arg.(term.Literal); ok {
			dt := l.Datatype()
			switch {
			case dt == term.XSDBoolean:
				if l.Lexical() != "true" && l.Lexical() != "false" && l.Lexical() != "0" && l.Lexical() != "1" {
					return nil // invalid boolean lexical form
				}
			case dt == term.XSDString || dt == (term.URIRef{}) || dt.Value() == "":
		...
		} else if _, ok := arg.(term.URIRef); ok {
			return nil // URI has no EBV
		} else if _, ok := arg.(term.BNode); ok {
			return nil // BNode has no EBV
		}
		return term.NewLiteral(!effectiveBooleanValue(arg))
	case "-":
		f := toFloat64(arg)
		if isIntegral(arg) {
			return term.NewLiteral(int64(-f))
		}
		return term.NewLiteral(-f)
	}
	return nil
}
...
func mergeBindings(a, b map[string]term.Term) map[string]term.Term {
	result := make(map[string]term.Term, len(a)+len(b))
	for k, v := range a {
		result[k] = v
	}
	for k, v := range b {
...
func copyBindings(b map[string]term.Term) map[string]term.Term {
	result := make(map[string]term.Term, len(b))
	for k, v := range b {
		result[k] = v
	}
	return result
...
func isCompatible(a, b map[string]term.Term) bool {
	for k, va := range a {
		if vb, ok := b[k]; ok {
			if va.N3() != vb.N3() {
				return false
...
func solutionKey(s map[string]term.Term, vars []string) string {
	var parts []string
	if vars == nil {
		for k, v := range s {
...
func termValuesEqual(a, b term.Term) bool {
	if a == nil || b == nil {
		return a == nil && b == nil
	}
	la, aIsLit := a.(term.Literal)
	lb, bIsLit := b.(term.Literal)
...
func isDecimal(t term.Term) bool {
	if l, ok := t.(term.Literal); ok {
		return l.Datatype() == term.XSDDecimal
	}
	return false
}
...
func isNumericDatatype(dt term.URIRef) bool {
	return dt == term.XSDInteger || dt == term.XSDInt || dt == term.XSDLong ||
		dt == term.XSDFloat || dt == term.XSDDouble || dt == term.XSDDecimal
}
...
func buildStorePattern(tp *Triple, prefixes map[string]string) term.TriplePattern {
	...
	if s != nil {
		if subj, ok := s.(term.Subject); ok {
			pat.Subject = subj
		}
	}
	p := resolvePatternTerm(tp.Predicate, nil, prefixes)
	if p != nil {
		if pred, ok := p.(term.URIRef); ok {
			pat.Predicate = &pred
		}
	}
