...
func (p *sparqlParser) parseCompareExpr() (Expr, error) {
	...
	if p.matchKeywordCI("IN") {
		...
		if len(list) == 0 {
			return &LiteralExpr{Value: term.NewLiteral(false)}, nil
		}
	...
	if p.matchKeywordCI("NOT") {
		...
		if p.matchKeywordCI("IN") {
			...
			if len(list) == 0 {
				return &LiteralExpr{Value: term.NewLiteral(true)}, nil
			}
			var result Expr = &BinaryExpr{Op: "!=", Left: left, Right: list[0]}
			for _, item := range list[1:] {
				result = &BinaryExpr{Op: "&&", Left: result, Right: &BinaryExpr{Op: "!=", Left: left, Right: item}}
		...
		if p.pos+1 < len(p.input) && p.input[p.pos+1] == '<' {
			...
			if p.pos < len(p.input) && p.input[p.pos] == '(' {
				...
				case *LiteralExpr:
					...
					if se.Value != nil {
						if _, isURI := se.Value.(term.URIRef); !isURI {
	...
	if name == "GROUP_CONCAT" {
		...
		if p.pos < len(p.input) && p.input[p.pos] == ';' {
			...
			if p.matchKeywordCI("SEPARATOR") {
				...
				p.skipWS()
				sep := p.readTermOrVar()
				fe.Separator = string(p.resolveTermValue(sep).(term.Literal).Lexical())
			}
		} else {
...
func (p *sparqlParser) parseExprOrTermInTriple() (Expr, error) {
	...
	if val == nil {
		return &LiteralExpr{Value: term.NewLiteral(t)}, nil
	}
	if u, ok := val.(term.URIRef); ok {
		return &IRIExpr{Value: u.Value()}, nil
	}
