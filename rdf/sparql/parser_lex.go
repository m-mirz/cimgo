...
func (p *sparqlParser) readTermOrVar() string {
	...
	if ch == 'a' && (p.pos+1 >= len(p.input) || !isNameChar(rune(p.input[p.pos+1]))) {
		p.pos++
		return "<" + namespace.RDF.Type.Value() + ">"
	}
...
func (p *sparqlParser) parseCollectionTriples() (string, []Triple, error) {
	p.pos++ // skip (
	rdfFirst := "<" + namespace.RDF.First.Value() + ">"
	rdfRest := "<" + namespace.RDF.Rest.Value() + ">"
	rdfNil := "<" + namespace.RDF.Nil.Value() + ">"
...
func (p *sparqlParser) readTermValue() term.Term {
	tv := p.readTermOrVar()
