...
func (p *sparqlParser) parseTriplePatterns() ([]Triple, error) {
	...
	for {
		...
		for {
			...
			p.skipWS()
			annTriples, annErr := p.parseAnnotationsAndReifiers(subj, pred, obj, predPath)
			if annErr != nil {
				return nil, annErr
			}
...
func (p *sparqlParser) parseAnnotationsAndReifiers(subj, pred, obj string, predPath paths.Path) ([]Triple, error) {
	var extra []Triple
	...
	for {
		...
		if p.input[p.pos] == '~' {
			...
			if p.pos+1 < len(p.input) && p.input[p.pos] == '{' && p.input[p.pos+1] == '|' {
				p.pos += 2
				annTriples, err := p.parseAnnotationBlock(reifierID)
				if err != nil {
					return nil, err
				}
...
func (p *sparqlParser) parseValues() (*ValuesPattern, error) {
	...
	for {
		...
		if p.input[p.pos] == '(' {
			...
			if p.matchKeywordCI("UNDEF") {
				p.pos += 5
				values = append(values, []term.Term{nil})
			} else {
				before := p.pos
				val := p.readTermValue()
				if p.tripleTermError != nil {
				...
				values = append(values, []term.Term{val})
			}
		}
	}
