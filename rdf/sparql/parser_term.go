...
func (p *sparqlParser) resolveTermValue(s string) term.Term {
	if s == "" {
	...
	if strings.HasPrefix(s, "<") && strings.HasSuffix(s, ">") {
		return term.NewURIRefUnsafe(s[1 : len(s)-1])
	}
			...
			if !strings.HasPrefix(dtPart, "<") {
				...
				if cidx := strings.Index(dtPart, ":"); cidx >= 0 {
					...
					if ns, ok := p.prefixes[prefix]; ok {
						...
						return term.NewLiteral(lit.Lexical(), term.WithDatatype(term.NewURIRefUnsafe(ns+local)))
					}
				}
			}
		}
...
func parseLiteralString(s string) term.Literal {
	...
	if strings.HasPrefix(rest, "@") {
		...
		if idx := strings.Index(langDir, "--"); idx >= 0 {
			...
			opts = append(opts, term.WithLang(langDir[:idx]))
			opts = append(opts, term.WithDir(langDir[idx+2:]))
		} else {
		...
		if strings.HasPrefix(dt, "<") && strings.HasSuffix(dt, ">") {
			opts = append(opts, term.WithDatatype(term.NewURIRefUnsafe(dt[1:len(dt)-1])))
		}
	}
	return term.NewLiteral(lexical, opts...)
}
