...
func TestSPARQLLangFunction(t *testing.T) {
	...
	g.Add(s, p, term.NewLiteral("hallo", term.WithLang("de")))
...
func TestSPARQLEffectiveBooleanValue(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/val")
	g.Add(s, p, term.NewLiteral(""))
	g.Add(s, p, term.NewLiteral("nonempty"))
	g.Add(s, p, term.NewLiteral(0))
