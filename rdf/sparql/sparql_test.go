...
func makeSPARQLGraph(t *testing.T) *graph.Graph {
	t.Helper()
	g := graph.NewGraph()
	g.Bind("ex", term.NewURIRefUnsafe("http://example.org/"))
	alice, _ := term.NewURIRef("http://example.org/Alice")
	bob, _ := term.NewURIRef("http://example.org/Bob")
	charlie, _ := term.NewURIRef("http://example.org/Charlie")
	...
	g.Add(alice, age, term.NewLiteral(30))
	g.Add(alice, knows, bob)

	g.Add(bob, namespace.RDF.Type, person)
	g.Add(bob, name, term.NewLiteral("Bob"))
	g.Add(bob, age, term.NewLiteral(25))
	g.Add(bob, knows, charlie)

	g.Add(charlie, namespace.RDF.Type, person)
	g.Add(charlie, name, term.NewLiteral("Charlie"))
	g.Add(charlie, age, term.NewLiteral(35))
	return g
}
...
func TestSPARQLStrlenUnicode(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/val")
	// "cafe\u0301" is 5 runes but 6 bytes; "日本語" is 3 runes but 9 bytes
	g.Add(s, p, term.NewLiteral("日本語"))
...
func TestSPARQLValueEqualityNumericCrossType(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/val")
	// Add integer 1
	g.Add(s, p, term.NewLiteral("1", term.WithDatatype(term.XSDInteger)))
...
func BenchmarkSPARQLSelectAll(b *testing.B) {
	...
	for i := 0; i < 100; i++ {
		...
		p, _ := term.NewURIRef("http://example.org/name")
		g.Add(s, p, term.NewLiteral(fmt.Sprintf("name%d", i)))
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
