...
func parseSRXBinding(b srxBinding) term.Term {
	if b.URI != "" {
		return term.NewURIRefUnsafe(b.URI)
	}
	if b.BNode != "" {
		return term.NewBNode(b.BNode)
	}
	if b.Literal != nil {
...
func parseSRXLiteral(lit *srxLiteral) term.Literal {
	var opts []term.LiteralOption
	if lit.Lang != "" {
		if idx := strings.Index(lit.Lang, "--"); idx >= 0 {
			// Legacy: direction embedded in lang tag as lang--dir
			opts = append(opts, term.WithLang(lit.Lang[:idx]))
			opts = append(opts, term.WithDir(lit.Lang[idx+2:]))
		} else {
			opts = append(opts, term.WithLang(lit.Lang))
		}
		// SPARQL 1.2: its:dir attribute takes precedence over --dir suffix
		if lit.Dir != "" {
			opts = append(opts, term.WithDir(lit.Dir))
		}
	} else if lit.Datatype != "" {
		opts = append(opts, term.WithDatatype(term.NewURIRefUnsafe(lit.Datatype)))
	}
	return term.NewLiteral(lit.Value, opts...)
}

func parseSRXTripleComponent(c srxTripleComponent) term.Term {
	if c.URI != "" {
		return term.NewURIRefUnsafe(c.URI)
	}
	if c.BNode != "" {
		return term.NewBNode(c.BNode)
	}
	if c.Literal != nil {
...
func parseSRXTriple(t *srxTriple) term.Term {
	...
	return term.NewTripleTerm(subj, pred, o)
}
...
func parseSRJValue(v srjValue) term.Term {
	switch v.Type {
	case "uri":
		return term.NewURIRefUnsafe(srjString(v.Value))
	case "bnode":
		return term.NewBNode(srjString(v.Value))
	case "literal", "typed-literal":
		var opts []term.LiteralOption
		if v.Lang != "" {
			if idx := strings.Index(v.Lang, "--"); idx >= 0 {
				opts = append(opts, term.WithLang(v.Lang[:idx]))
				opts = append(opts, term.WithDir(v.Lang[idx+2:]))
			} else {
				opts = append(opts, term.WithLang(v.Lang))
			}
		} else if v.Datatype != "" {
			opts = append(opts, term.WithDatatype(term.NewURIRefUnsafe(v.Datatype)))
		}
		if v.Dir != "" {
			opts = append(opts, term.WithDir(v.Dir))
		}
		return term.NewLiteral(srjString(v.Value), opts...)
	case "triple":
...
func resultEqualWithBnodes(a, b []map[string]term.Term) bool {
...
func bindingKeyWith(row map[string]term.Term, collapseBnodes bool) string {
	var parts []string
	for k, v := range row {
		val := ""
		if v != nil {
			if collapseBnodes {
				if _, ok := v.(term.BNode); ok {
					val = "_:BNODE"
					parts = append(parts, k+"="+val)
					continue
				}
			}
			if l, ok := v.(term.Literal); ok && isNumericDatatype(l.Datatype()) {
