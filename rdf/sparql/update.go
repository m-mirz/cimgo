...
func evalModify(ds *Dataset, op *ModifyOp, prefixes map[string]string) error {
	...
	if len(op.Using) > 0 {
		...
		usedNamed := make(map[string]*graph.Graph)
...
func getGraph(ds *Dataset, name string) *graph.Graph {
	if name == "DEFAULT" {
		return ds.Default
	}
...
func getOrCreateGraph(ds *Dataset, name string) *graph.Graph {
	if name == "DEFAULT" {
		return ds.Default
	}
	if g, ok := ds.NamedGraphs[name]; ok {
...
func clearGraph(g *graph.Graph) {
	g.Remove(nil, nil, nil)
}

func graphForQuad(ds *Dataset, graphName string) *graph.Graph {
	if graphName == "" {
		return ds.Default
	}
...
func graphForQuadSolution(ds *Dataset, graphName string, sol map[string]term.Term) *graph.Graph {
	if graphName == "" {
		return ds.Default
	}
...
func resolveModifyGraph(ds *Dataset, graphName, with string, sol map[string]term.Term) *graph.Graph {
	if graphName == "" {
		if with != "" {
