...
func mustParse(t *testing.T, input string) *graph.Graph {
	t.Helper()
	g := graph.NewGraph()
...
func covSerialize(t *testing.T, g *graph.Graph) string {
	t.Helper()
...
func TestCovSerializeTypedLiteralPrefixed(t *testing.T) {
	g := graph.NewGraph()
	xsd := term.NewURIRefUnsafe("http://www.w3.org/2001/XMLSchema#")
	g.Bind("xsd", xsd)
	...
	dt := term.NewURIRefUnsafe("http://www.w3.org/2001/XMLSchema#date")
	o := term.NewLiteral("2024-01-01", term.WithDatatype(dt))
	// Add a triple using an xsd: URI as object so trackNS picks it up
	g.Add(s, p, o)
	g.Add(s, term.NewURIRefUnsafe("http://example.org/dt"), dt)
	out := covSerialize(t, g)
...
func TestCovSerializeTripleTermWithPrefix(t *testing.T) {
	...
	asserts := term.NewURIRefUnsafe("http://example.org/asserts")
	g.Add(s, asserts, tt)
...
func TestCovSerializeListInvalidExtraPred(t *testing.T) {
	g := graph.NewGraph()
	bn := term.NewBNode("l1")
	g.Add(bn, namespace.RDF.First, term.NewLiteral("a"))
	g.Add(bn, namespace.RDF.Rest, namespace.RDF.Nil)
	g.Add(bn, term.NewURIRefUnsafe("http://example.org/extra"), term.NewLiteral("x"))
	s := term.NewURIRefUnsafe("http://example.org/s")
	g.Add(s, term.NewURIRefUnsafe("http://example.org/list"), bn)
	out := covSerialize(t, g)
...
func TestCovSerializeListCyclic(t *testing.T) {
	g := graph.NewGraph()
	bn1 := term.NewBNode("cyc1")
	bn2 := term.NewBNode("cyc2")
	g.Add(bn1, namespace.RDF.First, term.NewLiteral("a"))
	g.Add(bn1, namespace.RDF.Rest, bn2)
	g.Add(bn2, namespace.RDF.First, term.NewLiteral("b"))
	g.Add(bn2, namespace.RDF.Rest, bn1) // cycle
	s := term.NewURIRefUnsafe("http://example.org/s")
	g.Add(s, term.NewURIRefUnsafe("http://example.org/list"), bn1)
	out := covSerialize(t, g)
	_ = out
}
...
func TestCovSerializeLocalNameInvalid(t *testing.T) {
	g := graph.NewGraph()
	g.Bind("ex", term.NewURIRefUnsafe("http://example.org/"))
	s := term.NewURIRefUnsafe("http://example.org/has space")
	p := term.NewURIRefUnsafe("http://example.org/p")
	g.Add(s, p, term.NewLiteral("val"))
	out := covSerialize(t, g)
...
func TestCovSerializeRDFSLabel(t *testing.T) {
	...
	s := term.NewURIRefUnsafe("http://example.org/s")
	g.Add(s, namespace.RDF.Type, namespace.RDFS.Class)
	g.Add(s, namespace.RDFS.Label, term.NewLiteral("My Class"))
	g.Add(s, term.NewURIRefUnsafe("http://example.org/extra"), term.NewLiteral("x"))
	out := covSerialize(t, g)
...
func TestCovSerializePrefixedDatatype(t *testing.T) {
	g := graph.NewGraph()
	g.Bind("xsd", term.NewURIRefUnsafe("http://www.w3.org/2001/XMLSchema#"))
	s := term.NewURIRefUnsafe("http://example.org/s")
	p := term.NewURIRefUnsafe("http://example.org/p")
	// Add a triple that uses a xsd: namespace URI so prefix gets tracked
	xsdType := term.NewURIRefUnsafe("http://www.w3.org/2001/XMLSchema#dateTime")
	g.Add(s, namespace.RDF.Type, xsdType)
	g.Add(s, p, term.NewLiteral("2024-01-01", term.WithDatatype(term.NewURIRefUnsafe("http://www.w3.org/2001/XMLSchema#date"))))
	out := covSerialize(t, g)
	if !strings.Contains(out, "xsd:") {
...
func TestCovSerializeLargePrefixedOutput(t *testing.T) {
	...
	s := term.NewURIRefUnsafe("http://example.org/s")
	for i := 0; i < 5; i++ {
		p := term.NewURIRefUnsafe("http://example.org/p" + string(rune('a'+i)))
...
func TestCovSerializeMultiSubject(t *testing.T) {
	...
	p := term.NewURIRefUnsafe("http://example.org/p")
	for i := 0; i < 3; i++ {
		s := term.NewURIRefUnsafe("http://example.org/s" + string(rune('a'+i)))
		g.Add(s, p, term.NewLiteral("v"))
	}
	out := covSerialize(t, g)
...
func TestCovSerializeWriterError(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		...
		g.Add(s, term.NewURIRefUnsafe(pURI), term.NewLiteral("a-value-that-is-long-enough-to-contribute-significantly-to-buffer-filling"))
	}
...
func TestCovSerializeRDFTypeShorthand(t *testing.T) {
	...
	g.Bind("ex", term.NewURIRefUnsafe("http://example.org/"))
	s := term.NewURIRefUnsafe("http://example.org/s")
	g.Add(s, namespace.RDF.Type, term.NewURIRefUnsafe("http://example.org/Thing"))
	out := covSerialize(t, g)
...
func TestCovLabelTripleTerm(t *testing.T) {
	g := graph.NewGraph()
	g.Bind("ex", term.NewURIRefUnsafe("http://example.org/"))
	s := term.NewURIRefUnsafe("http://example.org/a")
	p := term.NewURIRefUnsafe("http://example.org/b")
	o := term.NewURIRefUnsafe("http://example.org/c")
	tt := term.NewTripleTerm(s, p, o)
	// Triple term as subject hits the default branch in label()
	assertP := term.NewURIRefUnsafe("http://example.org/asserts")
	g.Add(tt, assertP, term.NewLiteral("val"))
	out := covSerialize(t, g)
	if !strings.Contains(out, "<<(") {
...
func TestCovSerializeTrackNSTripleTerm(t *testing.T) {
	...
	s := term.NewURIRefUnsafe("http://example.org/s")
	p := term.NewURIRefUnsafe("http://example.org/p")
	innerS := term.NewURIRefUnsafe("http://example.org/a")
	innerP := term.NewURIRefUnsafe("http://example.org/b")
	innerO := term.NewURIRefUnsafe("http://example.org/c")
	tt := term.NewTripleTerm(innerS, innerP, innerO)
	g.Add(s, p, tt)
	out := covSerialize(t, g)
...
func TestCovSerializeWriteSubjectReferencedBNode(t *testing.T) {
	...
	g.Add(bn, p, term.NewLiteral("val"))
	g.Add(s1, q, bn)
...
func TestCovSerializeInlineBNodeWithRDFType(t *testing.T) {
	...
	g.Bind("rdf", term.NewURIRefUnsafe("http://www.w3.org/1999/02/22-rdf-syntax-ns#"))
	g.Bind("ex", term.NewURIRefUnsafe("http://example.org/"))
	bn := term.NewBNode("typed")
	s := term.NewURIRefUnsafe("http://example.org/s")
	p := term.NewURIRefUnsafe("http://example.org/p")
	g.Add(bn, namespace.RDF.Type, term.NewURIRefUnsafe("http://example.org/Type"))
...
func TestCovSerializeIsValidListNilPreds(t *testing.T) {
	...
	g.Add(bn, namespace.RDF.Rest, term.NewBNode("nonexistent"))
	s := term.NewURIRefUnsafe("http://example.org/s")
	g.Add(s, term.NewURIRefUnsafe("http://example.org/list"), bn)
	out := covSerialize(t, g)
...
func TestCovSerializeIsValidListMultiFirst(t *testing.T) {
	...
	bn := term.NewBNode("l1")
	g.Add(bn, namespace.RDF.First, term.NewLiteral("a"))
	g.Add(bn, namespace.RDF.First, term.NewLiteral("b")) // two firsts
	g.Add(bn, namespace.RDF.Rest, namespace.RDF.Nil)
	s := term.NewURIRefUnsafe("http://example.org/s")
	g.Add(s, term.NewURIRefUnsafe("http://example.org/list"), bn)
	out := covSerialize(t, g)
...
func TestCovSerializePrefixInvalidChars(t *testing.T) {
	g := graph.NewGraph()
	g.Bind("1bad", term.NewURIRefUnsafe("http://example.org/"))
	s := term.NewURIRefUnsafe("http://example.org/s")
	p := term.NewURIRefUnsafe("http://example.org/p")
	g.Add(s, p, term.NewLiteral("v"))
	out := covSerialize(t, g)
...
func TestCovSerializeMarkListNodesEmpty(t *testing.T) {
	...
	p := term.NewURIRefUnsafe("http://example.org/p")
	bn := term.NewBNode("l1")
	g.Add(bn, namespace.RDF.First, term.NewLiteral("only"))
	g.Add(bn, namespace.RDF.Rest, namespace.RDF.Nil)
	g.Add(s, p, bn)
	out := covSerialize(t, g)
	if !strings.Contains(out, "( ") {
...
func TestCovSerializeObjectURIRef(t *testing.T) {
	...
	g.Add(s, p, term.NewURIRefUnsafe("http://example.org/obj"))
	// Object is a URIRef with no prefix
	g.Add(s, p, term.NewURIRefUnsafe("http://other.org/obj"))
	out := covSerialize(t, g)
...
func TestCovSerializeTripleTermBNodeSubject(t *testing.T) {
	...
	s := term.NewURIRefUnsafe("http://example.org/s")
	g.Add(s, term.NewURIRefUnsafe("http://example.org/q"), tt)
	out := covSerialize(t, g)
...
func TestCovSerializeManyPredicates(t *testing.T) {
	...
	g.Add(s, namespace.RDF.Type, term.NewURIRefUnsafe("http://example.org/Class"))
	g.Add(s, namespace.RDFS.Label, term.NewLiteral("label"))
	g.Add(s, term.NewURIRefUnsafe("http://example.org/p1"), term.NewLiteral("v1"))
	g.Add(s, term.NewURIRefUnsafe("http://example.org/p2"), term.NewLiteral("v2"))
	g.Bind("rdfs", term.NewURIRefUnsafe("http://www.w3.org/2000/01/rdf-schema#"))
	g.Bind("rdf", term.NewURIRefUnsafe("http://www.w3.org/1999/02/22-rdf-syntax-ns#"))
	out := covSerialize(t, g)
...
func TestCovSerializeWriterErrorAtVariousPoints(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		s := term.NewURIRefUnsafe("http://example.org/subject-with-long-name-" + string(rune('A'+i%26)) + string(rune('a'+(i/26)%26)))
		p := term.NewURIRefUnsafe("http://example.org/predicate")
		g.Add(s, p, term.NewLiteral("value-that-fills-buffer-"+string(rune('0'+i%10))))
		g.Add(s, namespace.RDF.Type, term.NewURIRefUnsafe("http://example.org/Class"))
		g.Add(s, term.NewURIRefUnsafe("http://example.org/prop2"), term.NewLiteral("another-value"))
	}
	g.Add(term.NewBNode("bn1"), term.NewURIRefUnsafe("http://example.org/p"), term.NewLiteral("bnode-val"))
...
func TestCovSerializeWriterErrorMultiPredicates(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		p := term.NewURIRefUnsafe("http://example.org/pred" + string(rune('A'+i%26)) + string(rune('a'+(i/26)%26)))
		g.Add(s, p, term.NewLiteral("val"+string(rune('0'+i%10))))
	}
...
func TestCovSerializeOrderSubjectsClasses(t *testing.T) {
	...
	c1 := term.NewURIRefUnsafe("http://example.org/ClassB")
	c2 := term.NewURIRefUnsafe("http://example.org/ClassA")
	g.Add(c1, namespace.RDF.Type, namespace.RDFS.Class)
	g.Add(c2, namespace.RDF.Type, namespace.RDFS.Class)
	g.Add(term.NewURIRefUnsafe("http://example.org/other"), term.NewURIRefUnsafe("http://example.org/p"), term.NewLiteral("v"))
	out := covSerialize(t, g)
