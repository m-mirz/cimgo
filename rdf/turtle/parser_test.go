...
func parseTurtle(t *testing.T, input string) *graph.Graph {
	t.Helper()
	g := graph.NewGraph()
...
func TestParseTurtleAShorthand(t *testing.T) {
	...
	if !g.Contains(s, namespace.RDF.Type, thing) {
...
func TestParseTurtleEmptyCollection(t *testing.T) {
	...
	`)
	s, _ := term.NewURIRef("http://example.org/s")
	list, _ := term.NewURIRef("http://example.org/list")
	if !g.Contains(s, list, namespace.RDF.Nil) {
...
func TestParseTurtleBaseDirective(t *testing.T) {
	...
	s, _ := term.NewURIRef("http://example.org/s")
	p, _ := term.NewURIRef("http://example.org/p")
	if !g.Contains(s, p, term.NewLiteral("hello")) {
