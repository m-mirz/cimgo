...
func TestSerializerResolveSubjectPerformance(t *testing.T) {
	...
	g.Bind("ex", term.NewURIRefUnsafe("http://example.org/"))
	for i := 0; i < 100; i++ {
		s, _ := term.NewURIRef("http://example.org/s" + strings.Repeat("x", i))
		p, _ := term.NewURIRef("http://example.org/p")
...
func TestSerializerErrorPropagationInList(t *testing.T) {
	...
	n2 := term.NewBNode()
	g.Add(s, p, n1)
	g.Add(n1, namespace.RDF.First, term.NewLiteral("a"))
	g.Add(n1, namespace.RDF.Rest, n2)
	g.Add(n2, namespace.RDF.First, term.NewLiteral("b"))
	g.Add(n2, namespace.RDF.Rest, namespace.RDF.Nil)
...
func TestParserValidIRIUnicodeEscape(t *testing.T) {
	...
	p, _ := term.NewURIRef("http://example.org/p")
	if !g.Contains(a, p, term.NewLiteral("v")) {
