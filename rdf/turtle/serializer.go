...
type turtleState struct {
	...
	spoMap map[string]map[string][]term.Term
	...
	usedNS map[string]term.URIRef // prefix -> namespace
	...
	subjectMap map[string]term.Subject
}

func newTurtleState(g *graph.Graph) *turtleState {
	return &turtleState{
		g:          g,
		spoMap:     make(map[string]map[string][]term.Term),
		refs:       make(map[string]int),
		usedNS:     make(map[string]term.URIRef),
		listHeads:  make(map[string]bool),
		listNodes:  make(map[string]bool),
		serialized: make(map[string]bool),
		subjectMap: make(map[string]term.Subject),
	}
}

// preprocess collects triples, counts references, detects lists, and tracks used prefixes.
func (ts *turtleState) preprocess() {
	ts.g.Triples(nil, nil, nil)(func(t term.Triple) bool {
		sk := termKey(t.Subject)
		pk := termKey(t.Predicate)

		if ts.spoMap[sk] == nil {
			ts.spoMap[sk] = make(map[string][]term.Term)
		}
...
func (ts *turtleState) trackNS(t term.Term) {
	if tt, ok := t.(term.TripleTerm); ok {
	...
	uri := u.Value()
	ts.g.Namespaces()(func(prefix string, ns term.URIRef) bool {
		nsStr := ns.Value()
...
func (ts *turtleState) detectLists() {
	firstKey := termKey(namespace.RDF.First)
	restKey := termKey(namespace.RDF.Rest)
	nilKey := termKey(namespace.RDF.Nil)
...
func (ts *turtleState) orderSubjects() {
	typeKey := termKey(namespace.RDF.Type)
	classKey := termKey(namespace.RDFS.Class)

	var topSubjects []term.Subject
	var bnodeSubjects []term.Subject
	var otherSubjects []term.Subject
	...
	sortSubjects := func(ss []term.Subject) {
		slices.SortFunc(ss, func(a, b term.Subject) int {
	...
	slices.SortFunc(bnodeSubjects, func(a, b term.Subject) int {
...
func (ts *turtleState) resolveSubject(sk string) term.Subject {
...
func (ts *turtleState) writeSubject(w io.Writer, subj term.Subject) error {
	sk := termKey(subj)
	ts.serialized[sk] = true

	// Check if this BNode can be inlined (referenced 0 times)
	if _, isBNode := subj.(term.BNode); isBNode && ts.refs[sk] == 0 && !ts.listHeads[sk] {
...
func (ts *turtleState) writePredicates(w io.Writer, sk string, indent string) error {
	...
	for i, pk := range sortedPreds {
		...
		// Sort objects
		slices.SortFunc(objs, func(a, b term.Term) int {
			return term.CompareTerm(a, b)
		})
...
func (ts *turtleState) sortPredicates(preds map[string][]term.Term) []string {
	typeKey := termKey(namespace.RDF.Type)
	labelKey := termKey(namespace.RDFS.Label)
...
func (ts *turtleState) label(t term.Term) string {
	switch v := t.(type) {
	case term.URIRef:
...
func (ts *turtleState) predLabel(pk string) string {
	if pk == termKey(namespace.RDF.Type) {
		return "a"
	}
...
func (ts *turtleState) objectStr(t term.Term) (string, error) {
	switch v := t.(type) {
	case term.URIRef:
...
func (ts *turtleState) literalStr(l term.Literal) string {
	n3 := l.N3()
	...
	if l.Language() == "" && l.Datatype() != (term.URIRef{}) && l.Datatype() != term.XSDString {
...
func (ts *turtleState) qnameOrFull(u term.URIRef) string {
	uri := u.Value()
	bestPrefix := ""
	bestNS := ""
...
func (ts *turtleState) tripleTermStr(tt term.TripleTerm) (string, error) {
...
func (ts *turtleState) listStr(head term.BNode) (string, error) {
	var items []string
	restKey := termKey(namespace.RDF.Rest)
	firstKey := termKey(namespace.RDF.First)
	nilKey := termKey(namespace.RDF.Nil)
...
func (ts *turtleState) inlineBNode(b term.BNode) (string, error) {
	sk := termKey(b)
	ts.serialized[sk] = true
	preds := ts.spoMap[sk]
